	...
	<section class="col-span-full mt-2">
		...
		{#each VITALS as { icon, value, label, description }}
			<Checkbox bind:group={vitalsGroup} {value} name="vitals" class="mt-2" custom checked>
				<div
					class="grid w-full cursor-pointer grid-cols-2 rounded-lg border-2 border-gray-200 bg-gray-100 p-5 font-normal text-gray-500 hover:bg-gray-50 hover:text-gray-600 peer-checked:border-primary-600 peer-checked:bg-gray-50 peer-checked:text-gray-600 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-gray-300 dark:peer-checked:bg-gray-800 dark:peer-checked:text-gray-300"
				>
					<svelte:component this={icon} class="h-8 w-8 text-primary-500 dark:text-primary-400" />
